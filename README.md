...
**entirely on Vercel + Amazon Aurora PostgreSQL**.

- **No Cloud Run** required for the H0 demo.
- **No Railway** required for the H0 demo.
- **No Supabase** required for the H0 demo.
...
User (judge, no signup)
  ...
  → Vercel API Routes (/api/h0/*)
  → Amazon Aurora PostgreSQL through Prisma
  → Workflow Records · Agent Traces · Approvals · Audit Logs · Security Decisions
  → Dashboard Replay
```
...
# 4. Seed the preconfigured demo workspace + 3 demo workflows
pnpm h0:seed
pnpm h0:check
...
| Variable | Required for H0 | Purpose |
| --- | --- | --- |
| `H0_DEMO_MODE` | yes | Server-side demo mode |
| `NEXT_PUBLIC_H0_DEMO_MODE` | yes | Client-side demo mode + `/` → `/h0` redirect |
| `NEXT_PUBLIC_APP_NAME` | yes | `JackOps` |
| `NEXT_PUBLIC_APP_URL` | yes | Deployed Vercel URL |
| `DATABASE_URL` | yes | Aurora pooled/proxy URL (`sslmode=require`) |
| `DIRECT_URL` | yes | Aurora direct URL (used for migrations) |
| `OPENAI_API_KEY` | optional | Live AI output; empty = deterministic demo |
| `GEMINI_API_KEY` | optional | Live AI output; empty = deterministic demo |

Supabase / Cloud Run / Railway / `NEXT_PUBLIC_API_URL` are **not** required for
the H0 demo (legacy production paths only).
...
2. **Build Command:** `pnpm --filter @jak-swarm/db db:generate && pnpm build`
3. **Install Command:** `pnpm install`
4. Set the environment variables above (Production).
5. Deploy. Open the URL → it redirects to `/h0` (no login).
6. Run `pnpm h0:seed` against Aurora (from your machine or a one-off job) so the
   dashboard has data, then redeploy if needed. You can also click **Run Demo
   Workflow** in the UI to write fresh records into Aurora live.
...
- Workflow table + agent timeline with token usage
- Approval queue (HIGH-risk external email)
- Security & Governance panel (SECURITY_DECISION events)
- Audit replay panel
- Cost / usage ledger
- "Run Demo Workflow" success + refreshed table
- AWS RDS console showing the rows in Aurora PostgreSQL

See `docs/H0_DEMO_SCRIPT.md` for the full under-3-minute video script,
`docs/H0_ARCHITECTURE.md` for the architecture, `docs/H0_AURORA_SETUP.md` for
Aurora setup, and `docs/H0_DEVPOST_SUBMISSION.md` for the submission text.
